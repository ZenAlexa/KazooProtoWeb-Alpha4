...
- ✅ **第三步阶段2完成**: 依赖注入落地，全局变量从 8 降至 2，容器成为单一服务来源
- ✅ **文档体系重组**: docs/ 目录按阶段拆分，测试/指南归档
- ⏳ **进行中**: 第三步阶段3 —— ES Module 入口改造

**第三步阶段2快照（最新）**:
- 全局命名空间清理：8 个变量 → 2 个（window.app, window.container）↓75%
- 移除模块桥接脚本：ExpressiveFeatures 和 instrumentPresetManager 直接 import
- AudioIO 容器化：可通过 container.get('audioIO') 调试访问
- 测试验证：19/19 AppContainer 单元测试 + 7 项浏览器回归测试全部通过
- 详见：[docs/refactoring/stage2-complete.md](docs/refactoring/stage2-complete.md)

**下一步行动**:
1. **阶段3：ES Module 入口** — 重构 `index.html` 加载路径，移除非模块脚本标签，建立单一模块入口
2. **阶段4：覆盖率与 UIManager 接入** — 将 UI 更新逻辑迁移到事件系统，并将测试覆盖率提升至 40%+
3. **阶段5：Logger 统一** — 替换 console 调用为 Logger 工具（~260 处）
